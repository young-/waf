...
function log_record(method,url,data,ruletag)
    ...
    local UTC_TIME = ngx.utctime()
    local UTC_TIME = string.sub(UTC_TIME,1,10)..'T'..string.sub(UTC_TIME,12)..'.000Z'
    local log_json_obj = {
                 client_ip = CLIENT_IP,
    ...
    local LOG_LINE = string.gsub(LOG_LINE, 'timestamp', '@timestamp')
